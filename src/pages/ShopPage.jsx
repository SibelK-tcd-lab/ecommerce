...
const ShopPage = () => {
  ...
  const products = [
    ...
    { id: 7, img: "https://images.pexels.com/photos/1036622/pexels-photo-1036622.jpeg?auto=compress&cs=tinysrgb&w=400" }, // YENİ: Modern Genç Stil (id 7 yeniden güncellendi)
    ...
    <div className="w-full bg-white overflow-x-hidden">
      <div className="bg-[#FAFAFA] py-6 px-4 md:px-10">
        <div className="container mx-auto flex flex-col md:flex-row justify-between items-center gap-4">
          ...
          <div className="flex items-center gap-3 font-bold text-sm text-[#BDBDBD]">
      ...
      <div className="bg-[#FAFAFA] pb-12 px-4 md:px-10 border-b">
        <div className="container mx-auto grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4">
          {categories.map((cat, i) => (
            <div key={i} className="relative aspect-[3/4] cursor-pointer group overflow-hidden shadow-sm">
      ...
      <div className="py-6 border-b px-4 md:px-10">
        <div className="container mx-auto flex flex-col md:flex-row justify-between items-center gap-6">
          ...
          <div className="flex items-center gap-4 w-full md:w-auto justify-center">
            <button className="flex-1 md:flex-none bg-[#F9F9F9] border px-4 py-3 rounded text-[#737373] flex items-center justify-between gap-2">
              Popularity <ChevronDown size={16}/>
            </button>
            <button className="bg-[#23A6F0] text-white px-10 py-3 rounded font-bold shadow-md hover:bg-[#1a8ecd] transition-all">
              Filter
            </button>
          </div>
        </div>
      ...
      <div className="bg-[#FAFAFA] py-16 px-4 md:px-10 border-t">
        <div className="container mx-auto grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 items-center gap-8 md:gap-12 grayscale opacity-60">
          {brandLogos.map((logo, i) => (
            <div key={i} className="flex justify-center items-center h-12 transition-all hover:grayscale-0 hover:opacity-100">
