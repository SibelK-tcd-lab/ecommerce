...
const ShopPage = () => {
  ...
  return (
    <div className="bg-white min-h-screen font-sans antialiased">
      {/* 1. Başlık ve Breadcrumb - Mobil padding ve hizalama güncellendi */}
      <div className="bg-[#FAFAFA] py-8">
        <div className="container mx-auto px-6 md:px-20 flex flex-col md:flex-row justify-between items-center gap-4 text-center md:text-left">
      ...
      {/* 2. Kategori Kartları - Mobil boşluklar optimize edildi */}
      <div className="bg-[#FAFAFA] pb-12">
        <div className="container mx-auto px-6 md:px-20">
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4">
            {categories.map((cat) => (
              <div key={cat.id} className="relative group cursor-pointer overflow-hidden aspect-[4/5] shadow-sm rounded-sm">
      ...
      {/* 3. Filtre Barı - Mobilde dikey yerleşim ve padding düzenlendi */}
        ...
        <div className="container mx-auto px-6 md:px-20 flex flex-col md:flex-row justify-between items-center gap-8">
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="flex gap-2">
              <button className="p-3 border border-gray-200 rounded-md text-[#252B42] hover:bg-gray-50 transition-colors"><LayoutGrid size={18} /></button>
              <button className="p-3 border border-gray-200 rounded-md text-[#737373] hover:bg-gray-50 transition-colors"><List size={18} /></button>
            </div>
          </div>
      ...
      {/* 4. Ürün Listesi - Link sarmalaması ve mobil padding eklendi */}
      <div className="container mx-auto px-6 md:px-20 py-16">
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-x-8 gap-y-16">
          {allProducts.slice(0, 12).map((product) => (
            <Link to={`/shop/${product.id}`} key={product.id} className="group flex flex-col items-center text-center">
              {/* Ürün Görseli */}
        ...
        {/* Sayfalama (Pagination) - Mobilde taşmayı önlemek için ayarlandı */}
        <div className="flex justify-center mt-24">
          <div className="flex border border-[#BDBDBD] rounded-md overflow-hidden shadow-sm text-sm">
            <button className="px-4 md:px-6 py-4 bg-[#F3F3F3] text-[#BDBDBD] font-bold border-r border-[#BDBDBD]">First</button>
            <button className="px-4 md:px-5 py-4 border-r border-[#BDBDBD] bg-white text-[#23A6F0] font-bold">1</button>
            <button className="px-4 md:px-5 py-4 border-r border-[#BDBDBD] bg-[#23A6F0] text-white font-bold shadow-inner">2</button>
            <button className="px-4 md:px-5 py-4 border-r border-[#BDBDBD] bg-white text-[#23A6F0] font-bold">3</button>
            <button className="px-4 md:px-6 py-4 bg-white text-[#23A6F0] font-bold">Next</button>
          </div>
        </div>
      </div>

      {/* 5. Marka Logoları - Padding ve Grayscale ayarı */}
      <section className="bg-[#FAFAFA] py-16">
        <div className="container mx-auto px-6 md:px-20 flex flex-wrap justify-center lg:justify-between items-center gap-12 opacity-50 grayscale transition-all">
          <img src="https://upload.wikimedia.org/wikipedia/commons/a/a6/Logo_Hooli.svg" className="h-8 md:h-10 object-contain" alt="hooli" />
          <img src="https://upload.wikimedia.org/wikipedia/commons/b/b9/Lyft_logo.svg" className="h-8 md:h-10 object-contain" alt="lyft" />
          <img src="https://upload.wikimedia.org/wikipedia/commons/0/08/Stripe_logo%2C_2016.svg" className="h-8 md:h-10 object-contain" alt="stripe" />
          <img src="https://upload.wikimedia.org/wikipedia/commons/a/a9/Amazon_logo.svg" className="h-8 md:h-10 object-contain" alt="amazon" />
          <img src="https://upload.wikimedia.org/wikipedia/commons/e/e3/Reddit_logo.svg" className="h-8 md:h-10 object-contain" alt="reddit" />
        </div>
      </section>
    </div>
  );
