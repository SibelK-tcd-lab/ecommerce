...
const ProductCard = ({ product }) => {
  ...
  const handleCardClick = () => {
    // Ürünün benzersiz ID'sinin var olduğundan emin oluyoruz
  ...
  const handleLearnMore = (e) => {
    e.stopPropagation(); // Butona tıklandığında kartın tıklanmasını engeller
    const productId = product.id || product._id;
    if (productId) {
      history.push(`/product/${productId}`);
    }
  ...
  return (
    <div 
      ...
      className="flex flex-col md:flex-row bg-white shadow-sm border border-[#EBEBEB] overflow-hidden hover:shadow-md transition-all cursor-pointer"
    >
      ...
      <div className="w-full md:w-[55%] p-6 flex flex-col justify-between">
        ...
        <button 
          ...
          className="flex items-center gap-2 border border-[#23A6F0] text-[#23A6F0] font-bold py-2 px-5 rounded-full hover:bg-[#23A6F0] hover:text-white transition-all w-fit"
        >
